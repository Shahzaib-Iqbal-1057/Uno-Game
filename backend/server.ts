...
let users : user[] = [];
let deck_card : Card = {name : "",value : 0,color : ""};
let user_cards : userCards[] = [];
let turn : number = 0;
let uno_reverse_flag = false;
let deck : Card[] = [
...
const sendData = () => {
    for(let i = 0; i < 4; i++)
    {
        let all_names : string[] = users.map(x=>x.name);
        let cards : Card[] = [];
        for(let i = 0; i < 8; i++)
        {
        ...
        io.to(users[i].id).emit("player_names",{my_name : users[i].name,other_player_names : all_names.filter(x => x !== users[i].name)});
...
const cardCompatibility = (card : Card) : boolean => {
...
const giveDrawCards = (index : number,value : number) => {
    if(uno_reverse_flag)
    {
        if(index === 0)
        {
...
const calculateNextTurn = (turn : number,card : Card) : number => {
    if(card.color === "wild")
    {
        return turn;
    }
...
const putCardOnDeck = (card : Card,id : string) => {
...
const getCardFromDeck = (id : string) => {
    if(users[turn%4].id !== id)
    {
        return;
    }
