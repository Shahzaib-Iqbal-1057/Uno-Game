...
const currentTurns: Record<string, { turn: number; centerCard: Card; reverse: boolean }> = {};


const cardCompatibility = (playerCard: Card, centerCard: Card): boolean => {
    if (centerCard.color === "wild" || playerCard.color === "wild") return true;
    if (centerCard.color === playerCard.color) return true;
    if (playerCard.name.split(' ')[1].split('-')[0] === "num" &&
        centerCard.name.split(' ')[1].split('-')[0] === "num" &&
        playerCard.value === centerCard.value) return true;
    return false;
};

const getNextPlayerIndex = (currentIndex: number, numPlayers: number, reverse: boolean, step: number = 1): number => {
    if (reverse) {
...
const events = (io: Server) => {
    io.on("connection", (socket: Socket) => {
        ...
        socket.on("joinRoom", async (data: JoinData) => {
            ...
            if (!room) {
                ...
                room = await Room.create({
                    ...
                    playersJoined: 0,
                    players: [],
                    remainingDeck: shuffleDeck(deck)
                });
            }
            ...
            console.log(
                `Player ${name} joined room ${room.roomId} (${room.playersJoined}/${room.maxPlayers})`
            );
            ...
            socket.on("playCard", async (data: { roomId: string; card: Card }) => {
                ...
                if (!cardCompatibility(card, centerCard)) return;
            ...
            socket.on("pickFromDeck", async (data: { roomId: string }) => {
                const { roomId } = data;
                const room = await Room.findOne({ roomId });
                if (!room) return;
                ...
                if (room.remainingDeck.length === 0) return;
            ...
            socket.on("unoSignal", async (data: { roomId: string }) => {
                const { roomId } = data;
                const room = await Room.findOne({ roomId });
                if (!room) return;  
            ...
            socket.on("playerWon", async (data: { roomId: string }) => {
                const { roomId } = data;
                const room = await Room.findOne({ roomId });
                if (!room) return;
